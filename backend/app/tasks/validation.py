...
VALIDATION_CONFIG = {"min_score": 0.5, "timeout_seconds": 30, "max_retries": 3}
...
async def _validate_async(pid: str, session_id: str, prompt: str):
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to initialize validation session {session_id}: {e}"
            )
            ...
            try:
                ...
                try:
                    score, detail = await asyncio.wait_for(
                        asyncio.to_thread(
                            score_image_against_prompt, pin.image_url, prompt
                        ),
                    ...
                    await session_repo.add_log(
                        session_id, f"Pin {pin.id} evaluation timed out"
                    )
                    score = 0.0
                ...
                if score < VALIDATION_CONFIG["min_score"]:
                    ...
                    await session_repo.add_log(
                        session_id, f"Pin {pin.id} disqualified: {score}, {detail}"
                    )
                else:
                    await pin_repo.update_pin_status(pin.id, "approved")
                    await session_repo.add_log(
                        session_id, f"Pin {pin.id} approved: {score}, {detail}"
                    )

                # Store validation results in database
                await pin_repo.update_pin_match_score(pin.id, score)
                await pin_repo.update_pin_ai_explanation(
                    pin.id, detail.get("explanation", "")
                )
                ...
                await session_repo.add_log(
                    session_id, f"Validation failed for pin {pin.id}: {pin_error}"
                )
        ...
        try:
            ...
            print(
                f"Error updating prompt status after validation failure: {prompt_error}"
            )
